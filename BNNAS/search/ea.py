...
class EvolutionSearcher(object):
    ...
    def __init__(self, args):
        ...
        self.bn_matrix = torch.zeros([self.nr_layer, self.nr_state + 1], dtype=torch.double)
        for name, param in self.model.named_parameters():
            if '7.weight' in name:
                self.bn_matrix[int(name.split('.')[2]), int(name.split('.')[4])] = param.abs().mean().detach().item()
        print(self.bn_matrix)
    ...
    def get_bn_scores(self, cand):
        score = 0
        for i, arch in enumerate(cand):
            score += self.bn_matrix[i][arch]
        return -score
    ...
    def is_legal(self, cand):
        ...
        info['err'] = self.get_bn_scores(cand)
    ...
    def search(self):
        ...
        for cand in self.vis_dict:
            to_test_1000.append(cand)
            if len(to_test_1000) == 1000:
                break
        t1 = time.time()
        for cand in to_test_1000:
            self.get_bn_scores(cand)
        t2 = time.time()
        print('Time for testing 1000 candidates:')
        print(t2 - t1)
